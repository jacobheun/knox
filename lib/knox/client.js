...
Client.prototype.streamMultipart = function(request, destDir, fileName, fn) {
  var self = this;
      ...
      .on('end', function() {
        ...
        self.xmlParser.addListener('end', function(result) {
          if (result && result.UploadId) {
            ...
            console.log(resp.statusCode + " Data:\n"+xmlResponse);
          } else {
            req.emit("error", "Upload Id not returned");
          }
        });
...
Client.prototype.uploadPart = function(part, path) {
  ...
  self.partsReq.on('response',function(resp) {
    ...
    resp
      ...
      .on('end', function() {
        ...
        console.log("TAG:",resp.headers.etag);
  ...
  if (partToUpload) {
    ...
    self.partsReq.end();
    self.parts[self.currentPart] = null;
  }
